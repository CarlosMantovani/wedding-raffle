export const publicMessages = {
  genericError: 'Não foi possível concluir a operação. Tente novamente em alguns instantes.',
  quoteError: 'Não conseguimos atualizar o valor agora. Confira sua conexão e tente novamente.',
  checkoutError: 'Não conseguimos iniciar o pagamento. Tente novamente em alguns instantes.',
  recoveryError:
    'Não encontramos números para este telefone e código. Confira os dados e tente novamente.',
  drawClosed: 'Sorteio encerrado. Não é mais possível comprar números.',
  missingReference:
    'Não encontramos a referência da sua compra no retorno do pagamento. Tente novamente ou entre em contato com os noivos.',
  pending:
    'Seu pagamento ainda está sendo processado. Seus números serão gerados assim que a confirmação for recebida.',
  paymentStatusUnavailable:
    'Não foi possível confirmar o status neste momento. Seu pagamento pode continuar sendo processado e poderá ser confirmado depois.',
  paymentPollingStopped:
    'A consulta automática foi encerrada após cinco minutos. Seu pagamento ainda pode ser confirmado posteriormente.',
  paymentPollingOffline:
    'Você está sem conexão. A consulta foi pausada e será retomada de forma controlada quando a internet voltar.',
  rejected: 'Seu pagamento foi recusado. Verifique os dados ou tente outro método de pagamento.',
  cancelled: 'A transação foi cancelada. Nenhum número foi gerado para essa compra.',
  approvedWithoutNumbers:
    'Seu pagamento foi aprovado, mas os números não estão disponíveis. Entre em contato com o administrador.',
};
